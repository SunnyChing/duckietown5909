...
function solveMPC(l,n,m,T,z0,zT,zmax,umax,dt,step)
    ...
    #sum{(z[t,1]-zT[t,1])^2+(z[t,2]-zT[t,2])^2+10e-5*(z[t,3]-zT[t,3])^2  ,t=2:T+1})
      @setObjective(mpc, Min,
    sum{(z[T,1]-zT[1])^2+(z[T,2]-zT[2])^2+(z[T,3]-zT[3])^2+u[t,2]^2,t=1:T})
    ...
    for t = 1:T       
        ...
        @addNLConstraint(mpc, z[t+1,3] == z[t,3] + dt*u[t,1])
...
#--------------------Main function-----------------------------
function main()
    zT = [2;1.7;1/2*pi]  # Initial state - COO
    step=0

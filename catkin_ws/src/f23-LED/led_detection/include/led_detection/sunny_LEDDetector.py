...
class LEDDetector():
    ...
    def __init__(self, ploteverything=False, verbose=False, plotfinal=False, publisher=None):
        ...
        self.hsv_red3 = np.array([165,100,100])
        self.hsv_red4 = np.array([180,255,255])
        self.hsv_green1 = np.array([35,43,46])
        self.hsv_green2 = np.array([77,255,255])
        self.hsv_blue1 = np.array([100,43,36])
        self.hsv_blue2 = np.array([124,255,255])
        pass
    ...
    def detect_led(self,
        ...
        hsv = np.zeros(croppedshape)
        for i in range(n):
            channel[i,:,:] = cv2.cvtColor(images['rgb'][i,tly:bry,tlx:brx,:], cv2.COLOR_BGR2GRAY)
#SUNNY----------------------------------------------------------------------------
          #  hsvl[i,:,:] = cv2.cvtColor(images['rgb'][i,tly:bry,tlx:brx,:], cv2.COLOR_BGR2HSV)
          #  blue = cv2.inRange(hsv,self.hsv_blue1,self.hsv_blue2)
          #  green = cv2.inRange(hsv,self.hsv_green1,self.hsv_green2)

...
from rgb_led import RGB_LED, RGB_LED_Ardu
    ...
    def cycleTimer(self,event):
        ...
        if self.is_on:
            ...
            for i in range(5):
                ...
                self.is_on = True
# when Turn--------------------------------
    def cycleTimerTurnLeft(self,event):
        if not self.active:
            return
        if self.is_on:
            for i in [0,4]:
            ...
            self.led.setRGB(0, self.pattern[2])
            self.led.setRGB(4, [0,0,1])
            self.is_on = True
        
    def cycleTimerTurnRight(self,event):
        if not self.active:
            return
        if self.is_on:
            for i in [0,4]:
            ...
            self.led.setRGB(4, self.pattern[2])
            self.led.setRGB(0, [0,0,1])
            self.is_on = True
#-----------------------------------------------------------
	...
	if pattern_name not in ['TurnRight','TurnLeft']:
		self.changeFrequency()
	elif pattern_name == 'TurnLeft':
		self.changeFrequencyTurn()
	elif pattern_name == 'TurnRight':
		self.changeFrequencyTurn()
    ...
    def changeFrequencyTurn(self): 
        try:
                #self.cycle = msg.data
                self.cycle_timer.shutdown()
                #below, convert to hz
                d = 1.0/(2.0*self.cycle)
                if self.current_pattern_name == 'TurnLeft':
                        self.cycle_timer = rospy.Timer(rospy.Duration.from_sec(d), self.cycleTimerTurnLeft)
                else:
                        self.cycle_timer = rospy.Timer(rospy.Duration.from_sec(d), self.cycleTimerTurnRight)
        except ValueError as e:
                self.cycle = None
                self.current_pattern_name = None
                self.pub_state.publish(float(self.cycle)) 

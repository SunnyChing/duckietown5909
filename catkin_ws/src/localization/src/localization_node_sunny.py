...
class LocalizationNode(object):
    def __init__(self):
        self.node_name = 'localization_node_sunny'
	...
	self.pub_pose = rospy.Publisher("/duckie_pose", PoseWithCovarianceStamped ,queue_size=1, latch=True)
	...
	#Mr_w = self.transform_to_matrix(Tr_W.transform)
        ...
        if Tr_w is not None:
            ...
            self.lifetimer = rospy.Time.now()
	    self.publish_duckie_pose(T)
#SUNNY
    def publish_duckie_pose(self, t): 
	p=PoseWithCovarianceStamped()
	p.header.stamp = rospy.Time.now()
	p.header.frame_id="world"
	p.pose.pose.position=t.transform.translation

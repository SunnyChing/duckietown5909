...
class JoyMapper(object):
    def __init__(self):
        ...
        # Setup Parameters
        self.v_gain = self.setupParam("~speed_gain", 1)
        ...
        self.pub_mode = rospy.Publisher("~mode",String,queue_size=1)
        # Subscriptions
    ...
    def publishControl(self):
        ...
        if self.bicycle_kinematics:
            ...
            car_cmd_msg.angular.z = car_cmd_msg.v / self.simulated_vehicle_length * math.tan(steering_angle)
        else:
...
# logitek = 8, left joy = 9, right joy = 10
    ...
    def processButtons(self, joy_msg):
        ...
        elif (joy_msg.buttons[0] == 1): #manual mode
            mode_msg = String()
            rospy.loginfo('start manal mode')
            mode_msg.data = "MANUAL"
            self.pub_mode.publish(mode_msg)
        elif (joy_msg.buttons[1] == 1): #random mode
            mode_msg = String()
            rospy.loginfo('start random mode')
            mode_msg.data = "RANDOM"
            self.pub_mode.publish(mode_msg)

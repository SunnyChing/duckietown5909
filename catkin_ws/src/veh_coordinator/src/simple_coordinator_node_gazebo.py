...
import rospy
from duckietown_msgs.msg import CoordinationClearance, FSMState, BoolStamped, Twist2DStamped, IntersectionNodeArray, AprilTagsWithInfos
...
class VehicleCoordinator():
    ...
    def reconsider(self):
        if self.state == State.LANE_FOLLOWING and self.isSetIntersection :
            if self.mode == 'COORDINATION':
                self.reset_signals_detection()
                if self.traffic_light_intersection == True:
                    self.set_state(State.TL_SENSING)
                    #rospy.loginfo("[simple_coordinate]%s" %( self.traffic_light))
            ...
            if self.right_veh != SignalsDetection.NO_CAR or self.opposite_veh == SignalsDetection.SIGNAL_B or self.opposite_veh == SignalsDetection.SIGNAL_C:
                self.set_state(State.AT_STOP_CLEARING)
            elif self.time_at_current_state() > self.T_CROSS + self.T_SENSE:  #opppsite_veh is at signalA
    ...
    def cbTag(self, tag_msgs):
        ...
        if self.state == State.LANE_FOLLOWING:
             for taginfo in tag_msgs.infos:
                 if taginfo.id in self.trafficLightNode:
                    self.traffic_light_intersection = True
                 else:                
                    self.traffic_light_intersection = False
